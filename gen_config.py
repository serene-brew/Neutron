...
DEFAULTS = {
    ...
    # Kernel version string (e.g. "v1.0") packed into the NKRN header
    "kernel_version": '"v1.0"',
...
def main():
    ...
    # Kernel version: parse "vMAJOR.MINOR" into major, minor ints and a C string
    raw_kernel_version = cfg.get("kernel_version", DEFAULTS["kernel_version"])
    if raw_kernel_version.startswith('"') and raw_kernel_version.endswith('"'):
        raw_kernel_version = raw_kernel_version[1:-1]
    if len(raw_kernel_version) > 15:
        sys.stderr.write("Warning: kernel_version truncated to 15 characters\n")
        raw_kernel_version = raw_kernel_version[:15]
    kernel_version_str = '"' + raw_kernel_version + '"'
    # Strip optional leading 'v' and split into major/minor
    ver_stripped = raw_kernel_version.lstrip('vV')
    ver_parts = ver_stripped.split('.')
    try:
        kernel_version_major = int(ver_parts[0]) if len(ver_parts) > 0 else 1
        kernel_version_minor = int(ver_parts[1]) if len(ver_parts) > 1 else 0
    except ValueError:
        sys.stderr.write("Warning: could not parse kernel_version; defaulting to 1.0\n")
        kernel_version_major, kernel_version_minor = 1, 0
...
#define CFG_KERNEL_VERSION      {}
#define CFG_KERNEL_VERSION_MAJOR {}
#define CFG_KERNEL_VERSION_MINOR {}
...
""".format(
    ...
    with open(build_mk_path, "w") as mk:
        ...
        mk.write(f"KERNEL_VERSION_MAJOR := {kernel_version_major}\n")
        mk.write(f"KERNEL_VERSION_MINOR := {kernel_version_minor}\n")
